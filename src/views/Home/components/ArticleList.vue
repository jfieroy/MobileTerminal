...
// 第一页数据也是网络请求回来的，标签先挂载了，数据会来更新DOM，所以标签没有高度，list的load事件上来就触发
...
export default {
  ...
  methods: {
    ...
    async getArticleListFn () {
      ...
      this.list = [...this.list, ...res.data.data.results] // 合并 也可用concat
      // pre_timestamp
      // 下一段开头的那篇文章时间戳
      // 第一次 系统时间(timestamp) -> 后台 返回0~9条数据 ->携带第10条pre_timestamp值返回
      // 第二次(timestamp) - 上一次pre_timestamp(代表告诉后台，从指定时间戳往后再找10个) 10~19条 20条pre_timestamp返回
      // ......
      this.theTime = res.data.data.pre_timestamp // 分页保存

      // created()上来走一次，loading没触底本身就是false，但是给false也没问题
      // 为了一会儿onLoad()调用时，才有用
      this.loading = false // 如果不关闭，底部一直是加载中状态，下次触底不会再触发unLoad
      if (res.data.data.pre_timestamp === null) {
        // 本次回来的数据是最后的，没有下一次数据了
        this.finished = true
      }
    ...
    async onLoad () {
      ...
      // 第一个频道滚动到底部, 再切换到第二个频道的时候(新建-内容没有那么高), 滚动会从底部滚回到顶部
      // 这个时候发生了滚动, 所以滚动事件还是触发了, immediate-check无效
    ...
    async onRefresh () {
      // 目标：list数组清空，来一份新的数据
      this.list = []
      this.theTime = new Date().getTime // 页码回归初始化

      this.getArticleListFn() // 异步
    },
    // 反馈-不感兴趣
    async disLikeFn (id) {
      // 如果用try+catch自己处理错误，内部throw就不会向控制台抛出打印错误，而是交给catch内来自定义错误
      // try+catch捕获同步代码的异常
      try {
    ...
    // 文章单元格-点击事件  @click.native="itemClickFn(obj.art_id)"
    // itemClickFn (id) {
    //   this.$router.push({
