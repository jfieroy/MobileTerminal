<template>
  <div>
    <van-nav-bar title=" Coder - 登录"/>
    <div>
      <!-- van-form整体表单组件
        @submit -> 自定义事件(提交事件)  // @: 路径里的是文件夹，标签里的是事件
      ...
      <van-form @submit="onSubmit">
        ...
        <!-- round是否为圆形按钮
             ...
             block是否为块级元素
             type代表类型(有默认颜色)
             native-type代表原生button标签的type属性
...
// 组件使用套路
// 1. 找组件，引组件，用组件(标签和js复制)，读，删，改，加

// 目标1：实现顶部导航->自定义样式
// 目标2：实现表单组件->读，改，加
// 目标3：收集值以后，调用接口->查看登录结果
// 目标4：点击登录后给用户提示(正在登录中~)，防止用户频繁点击
  ...
  data () {
    return {
      user: {
        ...
        code: '246810' // 验证码(密码-必须是246810，无论手机号是什么)
      },
  ...
  methods: {
    ...
    async onSubmit (values) {
      ...
      try {
        ...
        // this.$router.push() 压栈(会产生历史记录，可以回退)，this.$router.replace() 替换(不会产生历史纪录)
        this.$router.replace({
          path: '/layout/home' // 因为路由规则里/layout里没有重定向，所以在这里写全
        })
      } catch (err) {
...
scoped尝试把此选择器后属性选择器匹配当前页面标签
(选不中部件内部的) */
// lang="less"当前style标签内是less语法
// 用/deep/就不会被vscode标红线
// /deep/会把属性选择器加到前面(用后代选择器的方式往里找匹配的类名)
// 结论：要修改组件内样式，如果你用了scoped，就需要在选择器前加/deep/即可
